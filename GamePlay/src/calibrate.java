...
public class calibrate {
	...
	public void generateOutput(){
		try{
			BufferedWriter output=new BufferedWriter(new FileWriter("calibration.txt"));
			float time=(float) ((1.0)*(endTime-startTime)/NumberOfNodes);
			output.write(""+time);
			output.close();
		}
		catch(Exception e){
	...
	public info minimax(int depth,int role,char[][] state,info cur,info alpha,info beta){
	    ...
	    if(NumberOfNodes==10000000)return cur;
		NumberOfNodes++;
		
		if(depth==10||isGameOver(state)==true){
